...
class TestFileFilterBasic:
    ...
    def test_init_with_defaults(self, tmp_path):
        ...
        file_filter = FileFilter(repo_path=tmp_path)
        
        assert file_filter.repo_path == tmp_path
        assert file_filter.scanner_files == set()
        assert file_filter.config_patterns == []
    ...
    def test_init_with_scanner_files(self, tmp_path):
        ...
        assert file_filter.scanner_files == scanner_files
    ...
    def test_init_with_config_patterns(self, tmp_path):
        ...
        assert file_filter.config_patterns == patterns
...
class TestFileFilterShouldSkip:
    ...
    def test_skip_scanner_file_exact_match(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("results.md")
    ...
    def test_skip_scanner_file_basename_match(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("subdir/results.md")
        assert should_skip is True
        assert reason == "scanner_file"
    ...
    def test_skip_config_pattern_match(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("README.md")
        assert should_skip is True
    ...
    def test_skip_config_pattern_txt(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("notes.txt")
        assert should_skip is True
    ...
    def test_no_skip_unmatched_file(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("main.cpp")
    ...
    def test_skip_config_pattern_full_path(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("docs/readme.txt")
        assert should_skip is True
    ...
    def test_skip_config_pattern_nested_path(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("src/test/unit/file.py")
        assert should_skip is True
    ...
    def test_skip_directory_pattern_tests(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("tests/test_main.py")
        assert should_skip is True
        assert "/*tests*/" in reason
        
        # Should match files in nested tests directory
        should_skip, reason = file_filter.should_skip("src/tests/test_utils.py")
        assert should_skip is True
        
        # Should not match files not in tests directory
        should_skip, reason = file_filter.should_skip("src/main.py")
        assert should_skip is False
    ...
    def test_skip_directory_pattern_vendor(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("vendor/lib/file.js")
        assert should_skip is True
        
        # Should match nested vendor directories
        should_skip, reason = file_filter.should_skip("packages/vendor/dep.js")
        assert should_skip is True
        
        # Should not match non-vendor paths
        should_skip, reason = file_filter.should_skip("src/utils.js")
        assert should_skip is False
    ...
    def test_skip_directory_pattern_with_wildcard(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("cmake-build-debug/CMakeCache.txt")
        assert should_skip is True
        
        # Should match cmake-build-release
        should_skip, reason = file_filter.should_skip("cmake-build-release/output.o")
        assert should_skip is True
        
        # Should not match other directories
        should_skip, reason = file_filter.should_skip("build/output.o")
        assert should_skip is False
    ...
    def test_skip_multiple_directory_patterns(self, tmp_path):
        ...
        # tests directory
        should_skip, _ = file_filter.should_skip("tests/test_main.py")
        assert should_skip is True
        
        # test directory (singular)
        should_skip, _ = file_filter.should_skip("test/unit_test.py")
        assert should_skip is True
        
        # vendor directory
        should_skip, _ = file_filter.should_skip("vendor/jquery.js")
        assert should_skip is True
        
        # 3rdparty directory
        should_skip, _ = file_filter.should_skip("3rdparty/boost/header.hpp")
        assert should_skip is True
        
        # Regular source files should not be skipped
        should_skip, _ = file_filter.should_skip("src/main.cpp")
        assert should_skip is False
...
class TestFileFilterGitignore:
    ...
    def test_load_gitignore(self, tmp_path):
        ...
        file_filter = FileFilter(repo_path=tmp_path, load_gitignore=True)
        
        assert file_filter._gitignore_spec is not None
    ...
    def test_skip_gitignored_file(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("module.pyc")
        assert should_skip is True
    ...
    def test_no_gitignore_file(self, tmp_path):
        ...
        should_skip, reason = file_filter.should_skip("something.pyc")
        assert should_skip is False
    ...
    def test_is_gitignored_method(self, tmp_path):
        ...
        assert file_filter.is_gitignored("file.pyc") is True
        assert file_filter.is_gitignored("file.o") is True
        assert file_filter.is_gitignored("file.cpp") is False
    ...
    def test_disabled_gitignore_loading(self, tmp_path):
        ...
        file_filter = FileFilter(repo_path=tmp_path, load_gitignore=False)
        
        assert file_filter._gitignore_spec is None
        assert file_filter.is_gitignored("file.pyc") is False
...
class TestFileFilterFilterPaths:
    ...
    def test_filter_paths_splits_correctly(self, tmp_path):
        ...
        kept, skipped = file_filter.filter_paths(paths)
    ...
    def test_filter_paths_empty_input(self, tmp_path):
        ...
        kept, skipped = file_filter.filter_paths([])
...
class TestFileFilterMutations:
    ...
    def test_add_scanner_files(self, tmp_path):
        ...
        file_filter = FileFilter(repo_path=tmp_path, scanner_files={"a.md"})
        
        file_filter.add_scanner_files("b.md", "c.log")
        
        assert "a.md" in file_filter.scanner_files
        assert "b.md" in file_filter.scanner_files
        assert "c.log" in file_filter.scanner_files

    def test_add_config_patterns(self, tmp_path):
        """Can add config patterns dynamically."""
        file_filter = FileFilter(repo_path=tmp_path, config_ignore_patterns=["*.md"])
        
        file_filter.add_config_patterns("*.txt", "*.json")
        
        assert file_filter.config_patterns == ["*.md", "*.txt", "*.json"]
    ...
    def test_reload_gitignore(self, tmp_path):
        ...
        file_filter = FileFilter(repo_path=tmp_path, load_gitignore=True)
        assert file_filter.is_gitignored("file.pyc") is True
        assert file_filter.is_gitignored("file.o") is False
        
        # Update .gitignore
        gitignore.write_text("*.o\n")
        file_filter.reload_gitignore()
...
class TestFileFilterWithoutPathspec:
    ...
    def test_handles_missing_pathspec(self, tmp_path):
        ...
        with patch("code_scanner.file_filter.HAS_PATHSPEC", False):
            ...
            file_filter.scanner_files = {"results.md"}
            should_skip, reason = file_filter.should_skip("results.md")
            assert should_skip is True
